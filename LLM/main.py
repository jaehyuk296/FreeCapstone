...
5.  (추가) 필터 조건이 1개일 경우, '$and' 없이 딕셔너리만 사용하세요. (예: {{"직업": "사무직"}})
...
7.  (복원) 사용자가 "30명", "10개" 등 명시적인 개수를 언급하면 "limit" 키로 해당 숫자를 추출하세요. 개수 언급이 없으면 5를 기본값으로 하세요.



8.  (수정) 'semantic_query'는 필터링 키워드를 제외한, 사용자의 핵심 의도를 나타내는 문장으로 생성하세요. 만약 의미 검색어가 없다면 "semantic_query"는 원본 쿼리 텍스트를 그대로 사용하세요.
...
def hybrid_search(search_query: SearchQuery):
    ...
    try:
        ...
        results = collection.query(
            ...
            include=["metadatas", "documents", "ids"] # (★★★ "ids" 포함 ★★★)
        ...
        try:
            ...
            found_ids = results.get('ids', [[]])[0] # (재시도 시에도 ID는 반환됨)
