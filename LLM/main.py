...
4.  여러 필터 조건이 있으면 ChromaDB의 '$and' 연산자 리스트로 묶으세요.
5.  만약 필터 조건이 없다면 "filters" 키의 값은 반드시 null 로 응답하세요. (빈 딕셔너리 {{}} 사용 금지)
6.  "30명", "10개" 등 개수 언급은 "limit" 키가 아닌 'semantic_query'에 포함시키세요. (ChromaDB의 n_results는 단순 상위 N개이며, 사용자가 원하는 '30명'과 다름)
7.  'semantic_query'는 필터링 키워드를 제외한, 사용자의 핵심 의도를 나타내는 문장으로 생성하세요. 만약 의미 검색어가 없다면 "semantic_query"는 원본 쿼리 텍스트를 그대로 사용하세요.
8.  오직 JSON 객체 형식으로만 응답해야 합니다.
9.  [중요] 응답은 반드시 "{" 로 시작하고 "}" 로 끝나야 합니다. 어떤 설명이나 인사말도 JSON 앞뒤에 붙이지 마세요.
...
출력: {{
    ...
    "semantic_query": "운동을 좋아하고 OTT를 시청하는 사람 10명"
}}
...
def hybrid_search(search_query: SearchQuery):
    ...
    try:
        ...
        # (★★★ 수정: limit 추출 로직 제거 ★★★)
        # limit = analysis_result.get("limit", 5) 
    ...
    try:
        ...
        results = collection.query(
            ...
            n_results=total_items_in_db, # (전체 데이터 검색)
            where=filters,               # (1번에서 추출한 필터 적용!)
            include=["metadatas", "documents", "distances"] # (거리 포함)
        )
        
        # (★★★ 추가: 유사도 임계값 필터링 ★★★)
        SIMILARITY_THRESHOLD = 0.5 # (튜닝 필요)
        ...
        if results.get('ids', [[]])[0]: 
            for i in range(len(results['ids'][0])):
                ...
                # 임계값(0.5)보다 거리가 가까운(유사한) 경우에만 결과에 추가
    ...
    # (★★★ 3.5. 메타데이터 1/0 값 텍스트로 변환 ★★★)
    print("⏳ 3.5. 메타데이터 1/0 값 텍스트로 변환 중...")
    ...
    if not found_docs:
        ...
        try:
            # (★★★ 수정: 검색된 결과가 너무 많으면 잘라서 전달 ★★★)
            MAX_CONTEXT_ITEMS = 20 # LLM에 한 번에 보낼 최대 문서 수 (예: 20개)
...
이 질문에 대해 DB에서 찾은 {len(found_docs)}개의 결과 중 상위 {len(found_docs_for_context)}개의 참고 자료는 다음과 같습니다.
