...
# '해외여행_희망여부' 열 생성: '해외여행 가고 싶지 않음'을 선택하지 않았으면 1, 선택했으면 0
# '해외여행 가고 싶지 않음' 컬럼이 없는 경우를 대비하여 df.get() 사용
df['해외여행_희망여부'] = (df.get('해외여행 가고 싶지 않음', 0) == 0).astype(int)
...
if '해외여행_희망여부' in final_df.columns:
    travel_wishers = final_df['해외여행_희망여부'].sum()
    if total_people > 0:
...
existing_destinations = [opt for opt in destination_options if opt in final_df.columns]

if existing_destinations:
    destination_counts = final_df[existing_destinations].sum().sort_values(ascending=False)
