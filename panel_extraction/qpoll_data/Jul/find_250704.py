...
# '포인트_신경쓰는_정도' 열 생성: 숫자 답변을 텍스트로 변환
df['포인트_신경쓰는_정도'] = df['문항1'].map(option_map)
...
if '포인트_신경쓰는_정도' in final_df.columns:
    point_counts = final_df['포인트_신경쓰는_정도'].value_counts()
    stats_summary.append("포인트 적립 신경쓰는 정도별 인원수:")
    stats_summary.append(point_counts.to_string())
    stats_summary.append("-" * 30)
...
try:
    with open(stats_full_path, 'w', encoding='utf-8') as f:
        f.write(f"📊 {FILE_ID} 포인트 적립 현황 요약 통계\n")
        f.write("-" * 30 + "\n")
