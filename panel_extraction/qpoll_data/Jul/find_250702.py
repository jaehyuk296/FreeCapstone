...
# '여행스타일' 열 생성: 숫자 답변을 텍스트로 변환
df['여행스타일'] = df['문항1'].map(option_map)
...
# --- 6. 최종 JSON 저장 ---
output_folder = '../../../json_extraction/qpoll_data/Jul/' # 7월 폴더로 경로 수정
...
if '여행스타일' in final_df.columns:
    style_counts = final_df['여행스타일'].value_counts()
    stats_summary.append("여행 스타일별 인원수:")
    stats_summary.append(style_counts.to_string())
    stats_summary.append("-" * 30)
...
# --- 8. 통계 결과를 .txt 파일로 저장 ---
stats_output_folder = '../../../json_extraction/qpoll_data/Jul/summary/' # 7월 폴더로 경로 수정
...
try:
    with open(stats_full_path, 'w', encoding='utf-8') as f:
        f.write(f"📊 {FILE_ID} 여행 스타일 요약 통계\n")
        f.write("-" * 30 + "\n")
