...
# --- 3. 데이터 전처리 ('비 오는 날 대처 방법' 맞춤) ---
option_map = {
    1: '그냥 비를 맞고 간다',
    2: '편의점에서 우산을 산다',
    3: '근처 비를 피할 수 있는 곳으로 들어간다',
    4: '가족/친구 등 주변인에게 연락한다',
    5: '기타'
}
...
# '비올때_대처방법' 열 생성: 숫자 답변을 텍스트로 변환
df['비올때_대처방법'] = df['문항1'].map(option_map)
...
print("☔️ '비 오는 날 대처 방법' 데이터 처리 완료.")
...
if '비올때_대처방법' in final_df.columns:
    action_counts = final_df['비올때_대처방법'].value_counts()
    stats_summary.append("대처 방법별 인원수:")
    stats_summary.append(action_counts.to_string())
    stats_summary.append("-" * 30)
...
try:
    with open(stats_full_path, 'w', encoding='utf-8') as f:
        f.write(f"📊 {FILE_ID} 비 오는 날 대처 방법 요약 통계\n")
        f.write("-" * 30 + "\n")
