...
# '주요_저장_사진' 열 생성: 숫자 답변을 텍스트로 변환
df['주요_저장_사진'] = df['문항1'].map(option_map)
...
if '주요_저장_사진' in final_df.columns:
    photo_counts = final_df['주요_저장_사진'].value_counts()
    stats_summary.append("주요 저장 사진 유형별 인원수:")
    stats_summary.append(photo_counts.to_string())
    stats_summary.append("-" * 30)
...
try:
    with open(stats_full_path, 'w', encoding='utf-8') as f:
        f.write(f"📊 {FILE_ID} 주요 저장 사진 요약 통계\n")
        f.write("-" * 30 + "\n")
