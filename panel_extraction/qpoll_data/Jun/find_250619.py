...
# '효과적인_다이어트_방법' 열 생성: 숫자 답변을 텍스트로 변환
df['효과적인_다이어트_방법'] = df['문항1'].map(option_map)
...
if '효과적인_다이어트_방법' in final_df.columns:
    method_counts = final_df['효과적인_다이어트_방법'].value_counts()
    stats_summary.append("효과적인 다이어트 방법별 인원수:")
    stats_summary.append(method_counts.to_string())
    stats_summary.append("-" * 30)
...
try:
    with open(stats_full_path, 'w', encoding='utf-8') as f:
        f.write(f"📊 {FILE_ID} 효과적인 다이어트 방법 요약 통계\n")
        f.write("-" * 30 + "\n")
