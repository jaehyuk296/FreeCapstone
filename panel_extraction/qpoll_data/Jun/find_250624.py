...
# '최대_지출처' 열 생성: 숫자 답변을 텍스트로 변환
df['최대_지출처'] = df['문항1'].map(option_map)
...
if '최대_지출처' in final_df.columns:
    spending_counts = final_df['최대_지출처'].value_counts()
    stats_summary.append("최대 지출처별 인원수:")
...
try:
    with open(stats_full_path, 'w', encoding='utf-8') as f:
        f.write(f"📊 {FILE_ID} 최근 최대 지출처 요약 통계\n")
        f.write("-" * 30 + "\n")
