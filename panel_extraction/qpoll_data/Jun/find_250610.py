...
# --- 3. 데이터 전처리 ('알람 설정 방식' 맞춤) ---
option_map = {
    1: '여러 개의 알람을 짧은 간격으로 설정',
    2: '한 개만 설정해놓고 바로 일어난다'
}

# '문항1' 열을 숫자형으로 변환 (숫자가 아닌 값은 NaN으로 처리)
df['문항1'] = pd.to_numeric(df['문항1'], errors='coerce')

# '알람_방식' 열 생성: 숫자 답변을 텍스트로 변환
df['알람_방식'] = df['문항1'].map(option_map)
...
if '알람_방식' in final_df.columns:
    alarm_counts = final_df['알람_방식'].value_counts()
    stats_summary.append("알람 설정 방식별 인원수:")
    stats_summary.append(alarm_counts.to_string())
    stats_summary.append("-" * 30)
...
try:
    with open(stats_full_path, 'w', encoding='utf-8') as f:
        f.write(f"📊 {FILE_ID} 알람 설정 방식 요약 통계\n")
        f.write("-" * 30 + "\n")
