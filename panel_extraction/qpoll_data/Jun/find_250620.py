...
# '아침식사_방식' 열 생성: 숫자 답변을 텍스트로 변환
df['아침식사_방식'] = df['문항1'].map(option_map)
...
if '아침식사_방식' in final_df.columns:
    method_counts = final_df['아침식사_방식'].value_counts()
    stats_summary.append("아침 식사 방식별 인원수:")
...
try:
    with open(stats_full_path, 'w', encoding='utf-8') as f:
        f.write(f"📊 {FILE_ID} 아침 식사 방식 요약 통계\n")
        f.write("-" * 30 + "\n")
