...
option_map = {
    ...
    3: '주 1회 정도',
    4: '월 1~2회 정도',
    5: '거의 하지 않거나 한 번도 해본 적 없다'
}

# '문항1' 열을 숫자형으로 변환 (숫자가 아닌 값은 NaN으로 처리)
df['문항1'] = pd.to_numeric(df['문항1'], errors='coerce')

# '혼밥_빈도' 열 생성: 숫자 답변을 텍스트로 변환
df['혼밥_빈도'] = df['문항1'].map(option_map)

# '혼밥_여부' 열 생성: 혼밥을 하면 1, 안 하면 0
df['혼밥_여부'] = (df['혼밥_빈도'] != '거의 하지 않거나 한 번도 해본 적 없다').astype(int)
...
if '혼밥_여부' in final_df.columns:
    solo_diners = final_df['혼밥_여부'].sum()
    if total_people > 0:
        stats_summary.append(f"혼밥 경험자 수: {solo_diners}명")
        stats_summary.append(f"혼밥 경험자 비율: {solo_diners / total_people * 100:.2f}%")
stats_summary.append("-" * 30)


if '혼밥_빈도' in final_df.columns:
    frequency_counts = final_df['혼밥_빈도'].value_counts()
    stats_summary.append("혼밥 빈도별 인원수:")
    stats_summary.append(frequency_counts.to_string())
    stats_summary.append("-" * 30)
