...
# '소비성향' 열 생성: 숫자 답변을 텍스트로 변환
df['소비성향'] = df['문항1'].map(option_map)
...
print(" minimalism '소비 성향' 데이터 처리 완료.")
...
if '소비성향' in final_df.columns:
    tendency_counts = final_df['소비성향'].value_counts()
    stats_summary.append("소비 성향별 인원수:")
    stats_summary.append(tendency_counts.to_string())
    stats_summary.append("-" * 30)
