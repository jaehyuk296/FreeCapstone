...
# '여름철_걱정거리' 열 생성: 숫자 답변을 텍스트로 변환
df['여름철_걱정거리'] = df['문항1'].map(option_map)
...
# --- 6. 최종 JSON 저장 ---
output_folder = '../../../json_extraction/qpoll_data/Jun/' # 6월 폴더로 경로 수정
...
if '걱정있음' in final_df.columns:
    worried_people = final_df['걱정있음'].sum()
    if total_people > 0:
        stats_summary.append(f"걱정거리가 있는 사람 수: {worried_people}명")
        stats_summary.append(f"걱정거리가 있는 사람 비율: {worried_people / total_people * 100:.2f}%")
stats_summary.append("-" * 30)


if '여름철_걱정거리' in final_df.columns:
    worry_counts = final_df['여름철_걱정거리'].value_counts()
    stats_summary.append("걱정거리 유형별 인원수:")
    stats_summary.append(worry_counts.to_string())
    stats_summary.append("-" * 30)
...
# --- 8. 통계 결과를 .txt 파일로 저장 ---
stats_output_folder = '../../../json_extraction/qpoll_data/Jun/summary/' # 6월 폴더로 경로 수정
...
try:
    with open(stats_full_path, 'w', encoding='utf-8') as f:
        f.write(f"📊 {FILE_ID} 여름철 걱정거리 요약 통계\n")
        f.write("-" * 30 + "\n")
